...
if (careers.length === 0) {
  ...
  sql += careers
    .map((career, i) => {
      return `  ('${escapeSQL(career.user_id)}', '${escapeSQL(career.title)}', '${career.started_at}', '${career.ended_at}', '${escapeSQL(career.description)}', '${escapeSQL(career.role)}', '${escapeSQL(career.scale)}')${i === careers.length - 1 ? '' : ','}`;
    })
    .join('\n');
